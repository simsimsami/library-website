...
// View contributors that have credit for specific books

export async function get_books_contributor(book_id) {
    try {
        const text = 'select * from books_contributor where book_id = $1';
        const values = [book_id];
        const response = (await getConnection()).query(text, values).catch(e => errorHandle(e));
...
export async function delete_contrib_role(contrib_role_title_id) {
    try {
        const text = "DELETE from contribution_role where contribution_role_id = $1";
        const values = [contrib_role_title_id];
        const response = (await getConnection()).query(text, values).catch(e => errorHandle(e));
        return (await response).rows;
    }
    catch (e) {
